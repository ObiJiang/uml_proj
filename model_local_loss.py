...
class MetaCluster():
    ...
    def model(self):
        ...
        # local_loss = tf.reduce_mean(diff_policy)
        ...
        grads_and_vars = tf.train.AdamOptimizer(learning_rate=self.lr).compute_gradients((self.alpha)*loss+(1-self.alpha)*local_loss)
    ...
    def train(self,data,labels,sess):
        ...
        for epoch_ind in range(30):
            ...
            loss,local_loss, _,_,miss_rate,predicted_label = sess.run([model.diff_policy, model.local_loss, model.keep_state_op,model.opt,model.miss_rate,model.predicted_label],feed_dict={model.sequences:data,model.labels:labels,model.nearest_index:nearest_index})
            # for gv in model.grads_and_vars:
            #     print(str(sess.run(gv[0],feed_dict={model.sequences:data,model.labels:labels,model.nearest_index:nearest_index})) + " - " + gv[1].name)
            ...
            # print(epoch_ind,loss,local_loss)
